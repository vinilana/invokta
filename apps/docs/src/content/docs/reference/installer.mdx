...
`remove` lets you select one or more managed installations, including a remote
...
No entries are preselected. Use the arrow keys to move, **Space** to select an
entry, or **A** to select all eligible entries. Press **Enter** to review the
selected engine/client pairs, then confirm once. Removal deletes only those
owned server entries and state records, preserving other installations and the
...
| --- | --- | --- |
| Antigravity | `agy` or `antigravity` | `~/.gemini/config/mcp_config.json` or an established `~/.gemini/antigravity/mcp_config.json` |
...
| Visual Studio Code | `code` | Linux: `~/.config/Code/User/mcp.json`; macOS: `~/Library/Application Support/Code/User/mcp.json`; remote server: its `data/User/mcp.json` |
...
Linux and Windows, and VS Code mutation is unsupported on Windows because both
clients use platform-specific configuration locations there.
...
On Windows, where no POSIX user id exists, the installer validates paths with
a Windows ownership identity (ADR 0027): symbolic links and junctions are
rejected, path identities are captured and revalidated before every write, and
containment inside the user profile is preserved, so a globally installed
engine package such as one under the npm global root installs and removes
normally. Exact POSIX file modes are not asserted on Windows; directory
confinement follows the user profile's access-control lists.
