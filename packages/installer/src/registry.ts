...
function appendPath(path: JsonPath, segment: string): string[] {
  return [...path, segment];
}
...
function validateAllStrings(value: unknown, issues: RegistryIssue[]): void {
  const work: Array<{ value: unknown; path: JsonPath }> = [{ value, path: [] }];
  while (work.length > 0) {
...
function validateUrl(
  ...
  if (url.pathname !== "/mcp" || value.includes("?") || value.includes("#")) {
    addIssue(issues, path, "INVALID_URL");
  }
  if (
    url.protocol === "http:" &&
...
function duplicateKeyIssues(text: string): RegistryIssue[] {
    ...
    if (frame.kind === "object") {
      if (frame.state === "key-or-end") {
        if (token.kind === "punctuation" && token.value === "}") {
          ...
          const keyPath = appendPath(frame.path, token.value);
          if (frame.keys.has(token.value))
            addIssue(issues, keyPath, "DUPLICATE_KEY");
...
export function validateRegistryBytes(
  ...
  validateAllStrings(parsed, issues);
  if (
    !validateObjectShape(
      parsed,
      [],
  ...
  if (!Array.isArray(parsed.entries)) {
    if (parsed.entries !== undefined)
      addIssue(issues, ["entries"], "INVALID_TYPE");
    return invalid(issues);
  }
  if (parsed.entries.length > registryEntryLimit) {
    addIssue(issues, ["entries"], "ARRAY_TOO_LONG");
  }
  ...
  for (let index = 0; index < parsed.entries.length; index += 1) {
    ...
    if (validateEntry(entry, index, issues, ids, serverNames)) {
  ...
  const entriesWithSourceIndex = validEntries.map((entry, sourceIndex) => {
    ...
    const compatibility = compatibilityRecord(descriptor, adapters);
    if (!Object.values(compatibility).some((result) => result.supported)) {
      addIssue(
        issues,
        ["entries", String(sourceIndex), "server", "transport"],
        "UNSUPPORTED_BY_ALL_TARGETS",
      );
    }
