...
| `AE-CREATE-OPENAPI-01..14` | JSON/YAML and local-reference fixtures cover OpenAPI 3.1 parsing, exact inclusive limits, deterministic endpoint discovery, all-eligible default selection, interactive numbered exclusion, repeatable canonical/operation-ID exclusion, unknown and ambiguous selectors, unsupported reasons, server and security precedence, request serialization, success response decoding, generated object-root schemas, fake-port execution, credential-free configuration, rollback, cancellation, and packed generated consumers through every selected adapter |
