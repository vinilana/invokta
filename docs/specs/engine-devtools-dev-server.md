...
The generated starter gains a `dev` script and a development dependency in a
later deliverable of the same specification, which changes starter snapshots
and is released as a creator change with release notes. Existing generated
projects are unaffected and can adopt the devtools by installing the package
and running `invokta-devtools serve dist/engine.js`.
