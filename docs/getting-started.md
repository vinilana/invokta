...
## Fast path: generate a runnable project

Use the complete profile to create a standalone project with all four entry
points, validate the project, and invoke its direct entry point:

```sh
npm create invokta-engine@latest welcome-engine -- --profile complete --yes
cd welcome-engine
npm run check
npm run direct -- Ada
```

The final command prints `{"message":"Welcome, Ada!"}`. Run `npm run devtools`
separately when you are ready for interactive development; it stays active and
prints the URL to open. The generated README maps the files and provides a
test-first checklist for replacing the welcome capability.

The steps below explain the code generated by that profile. They can also be
followed manually in an existing ESM TypeScript project.
...
Yarn 1.22.22 is required only when working in the Invokta repository. Install
and build from its root with:
...
The runnable entry point for the repository's expanded example is
[`examples/hello-engine/src/direct.ts`](../examples/hello-engine/src/direct.ts).
From a generated project, build and invoke the local entry point with:
...
The repository's runnable entry point is
[`examples/hello-engine/src/cli.ts`](../examples/hello-engine/src/cli.ts). A
generated complete or CLI project exposes the same commands through scripts:

```sh
npm run cli -- list
npm run cli -- describe onboarding.create-welcome-message
npm run cli -- run onboarding.create-welcome-message --input '{"name":"Ada"}'
```

JSON is the canonical output. Standard output contains only the requested
result; diagnostics belong on standard error. The CLI also accepts JSON through
standard input with the `--stdin` option.
...
accepted. A generated complete or MCP stdio project starts this entry point
with `npm run mcp:stdio`.
...
const explicitEnvFile = process.env.INVOKTA_ENV_FILE;
const envFile = explicitEnvFile ?? ".env";

if (!existsSync(envFile)) {
  if (explicitEnvFile !== undefined) {
    throw new Error("The configured environment file is unavailable.");
  }
} else {
...
Generated projects include `@invokta/devtools` and a build-and-watch script:

```sh
npm run devtools
```

Open the URL printed by the command; it uses `http://localhost:4100/` when that
port is available. The interface lists every capability with its input and
output fields plus the complete JSON Schemas, and invokes them from a
schema-seeded editor. The **Adapter** switch decides which execution path
